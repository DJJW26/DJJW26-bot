    ...
    async execute(message, args, client) {
        if (!args.length) {
            ...
            if (command) {
                ...
                helpbed.setTitle(`${command.name} Command`)
                helpbed.setColor('GREEN')
                helpbed.setDescription(`${command.description}`)
                helpbed.setThumbnail(message.author.displayAvatarURL())
                helpbed.addField('Category', `\`${command.category}\``)
                helpbed.addField('Aliases', `\`${command.aliases ? command.aliases.join(', ') : 'None'}\``)
                return message.reply({embeds: [helpbed]})
            } else {
